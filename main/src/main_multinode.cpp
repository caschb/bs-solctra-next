...
{
  spdlog::debug("Particles file: {}", particles_file);
...
int main(int argc, char **argv)
{
  ...
  if (my_rank == 0) {
    ...
    if (!handler.is_open()) {
      spdlog::error("Unable to open stdout.log for appending. Nothing to do.");
      exit(0);
    }
  ...
  if (my_rank == 0) {
    ...
    if (debug_flag != 0U) {
      auto endInitializationTime = MPI_Wtime();
      spdlog::info("Total initialization time=[{}].", endInitializationTime - startInitializationTime);
    }
  }
  ...
  if (my_rank == 0) { spdlog::info("Sending particles"); }
  ...
  if (my_rank == 0 && magprof != 0) {
    spdlog::info("Computing magnetic profiles");
  ...
  if (my_rank == 0) {
    startTime = MPI_Wtime();
    spdlog::info("Executing simulation");
  }
  ...
  if (my_rank == 0) {
    ...
    spdlog::info("Total execution time=[{}]", endTime - startTime);
    handler << "Total execution time=[" << (endTime - startTime) << "]." << std::endl;
    handler.close();
    handler.open("stats.csv", std::ofstream::out | std::ofstream::app);
    if (!handler.is_open()) {
      spdlog::error("Unable to open stats.csv for appending. Nothing to do");
      std::quick_exit(0);
    }
